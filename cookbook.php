...
try {
    ...
    // Fetch recipes from the user's cookbook
    $stmt = $db->prepare("
        SELECT recipes.*
        FROM cookbook
...
if (isset($_POST['remove_from_cookbook'])) {
    ...
    try {
        ...
        echo "Error removing recipe: " . htmlspecialchars($e->getMessage());
        exit();
    }
}
?>
...
<head>
    ...
    <title><?php echo $greetingMessage; ?>'s Cookbook</title>
    <style>
        .header-area {
            margin-bottom: 20px;
        }
...
<body>
    ...
    <div class="container">
        <div class="greeting-message">
            <h1 class="display-4"><?php echo $greetingMessage; ?>'s Cookbook</h1>
        </div>
                    ...
                    <div class="col-md-4">
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <form method="POST" action="cookbook.php" class="mt-2">
                                    <input type="hidden" name="recipe_id" value="<?php echo htmlspecialchars($recipe['recipe_ID']); ?>">
                                    <button type="submit" name="remove_from_cookbook" class="btn btn-danger">Remove from Cookbook</button>
                                </form>
                            </div>
                        </div>
                    </div>
